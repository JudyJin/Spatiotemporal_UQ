...
def maemis_loss(y_pred, y_true):
    ...
    # remove .cuda()
    loss1 = torch.max(y_pred.T[0].T-y_pred.T[1].T,torch.tensor([0.]))
    loss2 = torch.max(y_pred.T[1].T-y_true,torch.tensor([0.]))*2/pho
    loss3 = torch.max(y_true-y_pred.T[0].T,torch.tensor([0.]))*2/pho
